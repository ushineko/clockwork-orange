...
print(f"Test 1: Hourly, 1h ago -> {should_run(one_hour_ago, 'Hourly')}") # Should be True
print(f"Test 2: Hourly, 0.5h ago -> {should_run(now_ts - 1800, 'Hourly')}") # Should be False
print(f"Test 3: Daily, 25h ago -> {should_run(yesterday, 'Daily')}") # Should be True
print(f"Test 4: Daily, 2h ago -> {should_run(two_hours_ago, 'Daily')}") # Should be False
print(f"Test 5: Unknown -> {should_run(yesterday, 'Monthly')}") # Should be False (fallthrough)
