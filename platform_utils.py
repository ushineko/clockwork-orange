...
def _set_wallpaper_windows(image_path: Path) -> bool:
    ...
    try:
        ...
        if res:
            ...
            print(f"[ERROR] SystemParametersInfoW failed. Code: {ctypes.GetLastError()}")
            return False
    except Exception as e:
...
def _service_get_logs_linux() -> str:
    try:
        result = subprocess.run(
            ["journalctl", "--user", "-u", SERVICE_NAME_LINUX, "--no-pager", "-n", "50"],
...
def _service_install_windows(base_path: Path):
    if getattr(sys, 'frozen', False):
        ...
        if not _run_as_admin_windows(exe_path, "install"):
            raise RuntimeError("Failed to elevate for service installation. User may have cancelled UAC prompt.")
    else:
        raise RuntimeError("Service installation only supported in frozen (exe) mode on Windows.")

def _service_uninstall_windows():
    if getattr(sys, 'frozen', False):
        exe_path = sys.executable
        if not _run_as_admin_windows(exe_path, "remove"):
            raise RuntimeError("Failed to elevate for service uninstallation. User may have cancelled UAC prompt.")
    else:
        raise RuntimeError("Service uninstallation only supported in frozen (exe) mode on Windows.")

def _service_start_windows():
    if getattr(sys, 'frozen', False):
        exe_path = sys.executable
        if not _run_as_admin_windows(exe_path, "start"):
            raise RuntimeError("Failed to elevate for service start. User may have cancelled UAC prompt.")
    else:
        import win32serviceutil
...
def _service_stop_windows():
    if getattr(sys, 'frozen', False):
        ...
        if not _run_as_admin_windows(exe_path, "stop"):
            raise RuntimeError("Failed to elevate for service stop. User may have cancelled UAC prompt.")
    else:
        import win32serviceutil
...
def _service_get_logs_windows() -> str:
    ...
    log_file = Path.home() / "clockwork_service_test.log" # Make this consistent with service implementation
    if log_file.exists():
        try:
            # Read last 50 lines
